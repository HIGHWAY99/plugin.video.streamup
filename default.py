...
def PlayStreamUP(pageUrl='',Name='',Thumb='',roomId='',roomSlug='',plot='',liVe='',streamUrl='',streamkey=''):
	print "--DO A PLAYER SPLIT HERE--" ; debob(['pageUrl',pageUrl,'Name',Name,'Thumb',Thumb,'roomId',roomId,'roomSlug',roomSlug,'plot',plot,'liVe',liVe,'streamUrl',streamUrl]); 
	...
	if len(streamUrl) > 10: 
		...
		if len(streamkey)==0:
			...
			print 'temp streamkey',streamkey
			url='rtmp://%s/%s/%s' % ('66.55.92.79',roomId,streamkey)
			if (len(streamkey)==0) and (len(youtubekey)==0):
				print 'fetching youtubekey'
